...
var multiSend = function (transfers) {
    var i;
    var packetTransfers = [];
    for (i=0; i<transfers.length; i++) {
        packetTransfers.push(pack(transfers[i][0], transfers[i][i]));
    }

    for (var pos=0; pos < packetTransfers.length; pos += multiple) {
        var sendTransfers = packetTransfers.slice(pos, pos+multiple);
        sgt.multiTransfer(sendTransfers, {from: eth.accounts[0], gas: 3700000});
    }

    function pack(address, amount) {
        var addressNum = new BigNumber(address.substring(2), 16);
        var amount = web3.toWei(amount);
